...
public class BotInventoryScript : MonoBehaviour
{
    ...
    /// <summary>
    /// Method for bot to use when assigned the objective of trying to pickup a fallen weapon.
    /// </summary>
    public void AttemptToPickup()
    {
        RaycastHit hit;
        Debug.DrawLine(this.transform.position, this.transform.position + this.transform.forward * pickupRange, Color.blue, 5f);
        int mask = 1 << LayerMask.NameToLayer(Constants.WORLD_OBJECT_LAYER);
        if (Physics.Raycast(this.transform.position, this.transform.forward, out hit, pickupRange, mask))
        {
